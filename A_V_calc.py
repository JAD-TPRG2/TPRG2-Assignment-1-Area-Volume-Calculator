...
def area(view, length, wid) -> float:
    ...
    return round(r_area, 1)

def volume(view, length, wid, hei) -> float:
    ...
    return round(r_volume, 1)

def area_sphere(view, rad) -> float:
    ...
    return round(s_area, 1)

def volume_sphere(view, rad) -> float:
    '''Calculate the volume of a sphere.'''
    s_volume = float((4/3) * math.pi * rad**3)
    equation = f"(4/3) \u00d7 \u03c0 \u00d7 {rad}\u00b3 = {round(s_volume,1)}\u339d\u00b3"
    if view.lower() == "v":
        return round(s_volume, 1), equation
    return round(s_volume, 1)

def area_cylinder(view, rad, hei) -> float:
    ...
    return round(c_area, 1)

def volume_cylinder(view, rad, hei) -> float:
    '''Calculate the volume of a cylinder.'''
    c_volume = float(math.pi * rad**2 * hei)
    equation = f"\u03c0 \u00d7 {rad}\u00b2 \u00d7 {hei} = {round(c_volume,1)} \u339d\u00b3"
    if view.lower() == "v":
        return round(c_volume, 1), equation
    return round(c_volume, 1)

def area_pyramid(view, wid, length, hei) -> float:
    ...
    return round(p_area, 1)

def volume_pyramid(view, wid, length, hei) -> float:
    '''Calculate the volume of a pyramid.'''
    p_volume = float((wid * length * hei) / 3)
    equation = f"({wid} \u00d7 {length} \u00d7 {hei}) / 3 = {round(p_volume,1)} \u339d\u00b3"
    if view.lower() == "v":
        return round(p_volume, 1), equation
    return round(p_volume, 1)

def area_cone(view, rad, hei) -> float:
...
def main():
    ...
    while True: # Loop until user quits
        try:
            ...
            menu_input = input('Select calculation style:\n(V) Calculated view\n(D) Default view\n(Q) to Quit\n')
            ...
            if user == 1:
                ...
                rec_area = area(menu_input, rec_len, rec_wid)
                rec_volume = volume(menu_input, rec_len, rec_wid, rec_hei)
                if menu_input.lower() == 'v':
                    print(f'\nArea:{rec_area[0]}\u339d\u00b2 {rec_area[1]}\nVolume:{rec_volume[0]}\u339d\u00b3 {rec_volume[1]}')
                else:
                    print(f'\nThe Area of the Rectangle is {rec_area}\u339d\u00b3\nThe Volume of the Rectangle is {rec_volume}\u339d\u00b3')
            # Calculate the area and volume of sphere
            elif user == 2:
                sphere_rad = user_input('What is the radius of the Sphere in \u339d: ')
                sphere_area = area_sphere(menu_input, sphere_rad)
                sphere_volume = volume_sphere(menu_input, sphere_rad)
                if menu_input.lower() == 'v':
                    print(f'\nArea: {sphere_area[0]}\u339d\u00b2 ({sphere_area[1]})\nVolume: {sphere_volume[0]}\u339d\u00b3 ({sphere_volume[1]})')
                else:
                    print(f'\nThe Area of the Sphere is {sphere_area} u339d\u00b3\nThe Volume of the Sphere is {sphere_volume} \u339d\u00b3')
            # Calculate the area and volume of cylinder
                ...
                cyl_area = area_cylinder(menu_input, cyl_rad, cyl_hei)
                cyl_volume = volume_cylinder(menu_input, cyl_rad, cyl_hei)
                if menu_input.lower() == 'v':
                    print(f'\nArea: {cyl_area[0]}\u339d\u00b2 ({cyl_area[1]})\nVolume: {cyl_volume[0]}\u339d\u00b3 ({cyl_volume[1]})')
                else:
                    print(f'\nThe Area of the Cylinder is {cyl_area} \u339d\u00b3\nThe Volume of the Cylinder is {cyl_volume} \u339d\u00b3')
            # Calculate the area and volume of pyramid
                ...
                pyr_area = area_pyramid(menu_input, pyr_wid, pyr_len, pyr_hei)
                pyr_volume = volume_pyramid(menu_input, pyr_wid, pyr_len, pyr_hei)
                if menu_input.lower() == 'v':
                    print(f'\nArea: {pyr_area[0]}\u339d\u00b2 ({pyr_area[1]})\nVolume: {pyr_volume[0]}\u339d\u00b3 ({pyr_volume[1]})')
                else:
                    print(f'\nThe Area of the Pyramid is {pyr_area} \u339d\u00b3\nThe Volume of the Pyramid is {pyr_volume} \u339d\u00b3')
            # Calculate the area and volume of cone
                ...
                cone_area = area_cone(menu_input, cone_rad, cone_hei)
                cone_volume = volume_cone(menu_input, cone_rad, cone_hei)
                if menu_input.lower() == 'v':
                    print(f'\nArea: {cone_area[0]}\u339d\u00b2 {cone_area[1]}\nVolume: {cone_volume[0]}\u339d\u00b3 ({cone_volume[1]})')
                else:
                    print(f'\nThe Area of the Pyramid is {cone_area} \u339d\u00b3\nThe Volume of the Pyramid is {cone_volume} \u339d\u00b3')
